...
Alternatively, change two lines in the =backend Web App=[fn:3] to allow recording
selection from any device. Replace =mydomain= in the patches below:
#+begin_src shell -n
...
index 2d75b5e0ab..cca056c338 100644
...
@@ -68,7 +68,7 @@
                         ...
                         <ng-template #title>
-                            {{video.Title}}
+                            <a href="{{URLencode('http://mydomain/mythtv-stream-hls-dash/index.php?filename=' + video.FileName)}}" target="_blank">{{video.Title}}</a>
                         </ng-template>
                     </td>
