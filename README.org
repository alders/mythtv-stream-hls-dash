...
- The new Web Application ([[https://www.mythtv.org/wiki/Web_Application][MythTV v34]]) does not yet provide HTTP streaming
  support.
...
- Support for MythVideo.
- Support for live HDHomeRun streaming without MythTV.

How:
- Encode MythTV recordings and MythVideo providing playlist types =live=, =event= and =VOD=.
...
The installation below is based on Apache and Fedora. When using a different web
server / distribution locations and content user may be different. Adapting the
code is left as an exercise to the user.
...
- MythTV (for looking up the name of each recording based on filename and
  optionally for commerical cut info)
  - Any version up to and including version v0.34
...
On Fedora install:
#+begin_src shell -n
sudo dnf install ffmpeg mediainfo inotify-tools screen hdhomerun-devel sed
#+end_src
...
The =php= script generates a =bash= script. The commands are run as user =apache= via =sudo=:
#+begin_src shell -n
...
Optionally change a few lines in the =backend Web App=[fn:3] to allow recording
and / or video selection from any device. Replace =mydomain= in the patches below.

To allow recording selection for playback on any device:
#+begin_src shell -n
...
To allow video selection for playback on any device.
#+begin_src shell -n
...
To allow tv channel selection for playback on any device.
#+begin_src shell -n
...
Run the npm build script and install the web application:
#+begin_src shell -n
...
User interface options:
- Select *Stop streaming* when you are done watching[fn:18].
...
[fn:2] May depend on your distribution (e.g. 'data-www' is used for Ubuntu). May require one to adapt the =php= script.

[fn:3] This requires mythtv v34.

[fn:4] Leave as is since we are defining the settings for this recording.
...
[fn:7] This option is only visible when subtitles are available in the video file.
