...
#+ATTR_LATEX: :width 450px :options angle=90
...
=HLS event= and =HLS VOD=. The video should load automagically within 30 seconds. If
this does not happen, select either of the buttons to start playing. As a last
resort one could reload the web page.
...
- Integrate HTTP streaming into new [[https://www.mythtv.org/wiki/Web_Application][Web Application]] UI.
