...
- Support for live tv, live recording and recorded content.
...
- Shutdown lock can be used to prevent MythWelcome from shutting down.
...
The =php= script generates a =bash= script. The commands are run as user =apache= using =sudo=:
#+begin_src shell -n
...
and / or video and / or live tv selection from any device. Replace =mydomain= in
the patches below to your combined web server / =Mythbackend= address.
