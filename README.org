...
on the "Recorded Programs" page to =Stream HLS DASH= button.
...
User interface options[fn:6]:
- Select an available recording from the list box[fn:7].
- Select the =ABR= renditions from the select dropdown list box.
- Select the HW acceleration from the list box[fn:8].
- Select if the =Cutlist= should be used using the list box[fn:9].
- Select using the checkbox if =Subtitles= should be created[fn:10].
...
The first button =Delete Video Files= basically does what is says[fn:12],
...
available the player automatically tries to load the video[fn:13].
...
Two additional buttons appeared in Figure 6 for =Cleanup Video Files= [fn:14] and
=Download MP4=.
...
- Select the HW acceleration from the list box[fn:8].
- Select the TV channel from the list box[fn:16].
...
User interface options:
- Select *Stop streaming* when you are done watching[fn:17].
...
- The dynamic button next it indicates the =HLS= (no DASH support yet) manifest
  file is generated by showing the selected channel name.
...
[fn:7] /Leave as is since we are defining the settings for this recording./

[fn:8] Only VAAPI and no HW acceleration has been tested. Feedback on acceleration is appreciated.

[fn:9] /This option is only visible in the UI when a/ =Cutlist= /is defined in MythTV./

[fn:10] /This option is only visible when subtitles are available in the video file./

[fn:11] /Either one of the two or none at all./

[fn:12] /This will not delete any file from MythTV or change the MySQL database. All files can be recreated as long as the recording is available in MythTV./

[fn:13] /Automatic loading currently only works for live streaming. If no still of the video is shown after 30 seconds, as the case in this example, reload the browser page and start the video for viewing./

[fn:14] /This button is only shown when both playlist types/ =event= /and/ =VOD= /were selected as shown in Figure 1./

[fn:15] /The/ =cutlist= /itself was defined in MythTV which is translated into the
inpoint's and outpoint's of the/ =cutlist= /for the video./

[fn:16] The channel information is extracted from MythTV automagically.
