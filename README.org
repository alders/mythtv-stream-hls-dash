...
The backend code generates =bash= scripts. The commands in the scripts are run as user =apache=[fn:2] using =sudo=.
#+begin_src shell -n
...
- $xml -- Make sure your [[https://www.mythtv.org/wiki/Config.xml][Config.xml]] is readable by =$webuser=.
...
=mythfrontend=. Remuxing may also be required when otherwise the input video
format cannot be processed (e.g. =avi=).
