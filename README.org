...
- The new [[https://www.mythtv.org/wiki/Web_Application][Web Application]] (available since MythTV v34) interface does not yet provide HTTP
  streaming support.
...
- MythTV
  - Basically any version can be used. However, only version v0.34 and later, when installed from source, can offer
...
One can install MythTV using dnf as shown. However, as mentioned, the complete experience can only be achieved by installing from [[https://www.mythtv.org/wiki/Build_from_source][source]].
#+begin_src shell -n
...
Optionally change a few lines in the  [[https://www.mythtv.org/wiki/Web_Application][Web Application]][fn:4] to allow recording
...
- Select if the =Cutlist= should be used using the list box[fn:8].
- Select using the checkbox if =Subtitles= should be created[fn:9].
...
The first button =Delete Video Files= basically does what is says[fn:11].
...
the player automatically tries to load the video[fn:12].
...
Two additional buttons appeared in Figure 6, =Cleanup Video Files= [fn:13] and
=Download MP4=.
...
[fn:20] The channel information is extracted from MythTV automagically.
